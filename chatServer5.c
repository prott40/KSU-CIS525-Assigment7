    ...
    while ((bytes = SSL_read(ssl, buffer, MAX)) > 0)
    {
        ...
        else
        {
            ...
            case '0': // client quitting
                ...
                SSL_free(ssl);
                free(client);
            case '1': // setting username
                char uname[MAX];
                snprintf(uname, sizeof(uname), &buffer[1]);
                struct client *cp;
                //Check each client to make sure the name is a d
                LIST_FOREACH(cp, &client_list, clients){

                }
            case '2': // any other message
            }
        }
        char outmes[MAX];
